...
class Dossierprime extends Component {
    ...
    render(){
        if(this.state.api_key === false){
            ...
            let docs = [];
            this.state.data.documents.map((value, index)=> {
                return docs.push(<h4 className={'item_menu_gauche '+(index === 0 ? 'left-active' : '')} key={index}
                                     id={index+'pp'}>{value.type}</h4>);
            })
            return (
                <div>
                            ...
                            <div className="tile">
                                <div className="tile is-parent is-vertical">
                                    <div className="tile is-child notification has-text-centered">
                                        <p className="title">Dossier N° {this.state.data.id_dossierprime}</p>
                                    ...
                                    <div className="tile is-child">
                                        <div className="content has-text-centered">
                                            <button className="button is-primary is-outlined is-medium">Terminer
                                            </button>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="container footersonergia">
                        <div className="buttons has-addons is-centered">
                            <span className="button is-primary is-outlined is-medium"
                                  onClick={e => this.onOpenModal('moa')}>MOA</span>
                            <span className="button is-primary is-outlined is-medium"
                                  onClick={e => this.onOpenModal('moe')}>MOE</span>
                            <span className="button is-primary is-outlined is-medium"
                                  onClick={e => this.onOpenModal('travaux')}>TRAVAUX</span>
                        </div>
                    </div>
                    <ModalMoa open={this.state.open_moa} moaValues={this.state.data.moa}
                              onCloseModalType={this.onCloseModalType}/>
                    <ModalMoe open={this.state.open_moe} moeValues={this.state.data.moe}
                              onCloseModalType={this.onCloseModalType}/>
                    <ModalTravaux open={this.state.open_travaux} travauxValues={this.state.data.travaux}
                              onCloseModalType={this.onCloseModalType}/>
                </div>
            )
        }
