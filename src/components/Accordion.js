...
class  Accordion extends Component {
    ...
    currentAccordion= e =>{
        ...
        var els = document.querySelectorAll('.left-active')
        ...
        if(target.classList.contains('is-active')){
            let title_left = document.getElementById(this.state.numero+'pp')
    ...
    render(){
        let input= this.state.val.point_controle.map((value, index)=> {
            let checked = '';
            if(value.controle_valide){
                checked = ' checked ';
            }
            return (<div key={index}><input type="checkbox" name={value.id_controle} defaultChecked={checked} /><label htmlFor={value.id_controle}> {value.nom_controle}</label></div>);
        })
        return(
            <div className="divAccordion">
                <article className={"accordion "+(this.state.numero === 0 ? 'is-active' : '')}>
                    <div className={'accordion-header ' +this.state.color}>
                        <div><button className="toggle togglesonergia" aria-label="toggle" onClick={e => this.currentAccordion(e)}></button> {this.state.val.type}</div><div className="floatlink"><a href="https://www.google.fr" target="_blank"  rel="noopener noreferrer">{this.state.val.name_file}</a></div>
                    </div>
